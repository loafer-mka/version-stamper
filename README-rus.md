...
- проще всего использовать "смешанный" способ представления имён файлов,
  ...
  Posix-совестимых средах обычно понимают такой способ, и даже некоторые
  Windows программы - тоже (чаще всего при передаче аргументов приложениям
  Windows путь всё-таки надо будет  преобразовать).

- иногда возможны трюки, позволяющие из одной иметь доступ по пути в
  ...
  и разместив там ссылки на корни дисков Windows. Делаь это надо с правами
...
`git-for-windows` (то есть `MSYS2`) приведёт к копировнию в папку `foo`
содержимого всего диска `C:` - а если папка `foo` также находится на `C:`,
то к ошибке чересчур глубокой рекурии ... но в обоих варинтах это не то,
что вы хотели.
...
этого надо быть администраторам. А дл ярядового пользователя доступны лишь
"junction".
