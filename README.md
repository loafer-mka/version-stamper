...
7. `textoize` defines the option for version text representation format.
   If the value is not assigned (commented out), then the normal
   representation is "v1.2.333-branchname". If specified value `default`,
   then the generated string looks the same, but the `branchname` is
   shortened in some cases: instead of some prefixes `feature-`, `hotfix-`,
   `fix-`, `issue-`, `release-` are used shortened variants `f:`, `x:`,
   `x:`, `#`, and `R:`. If option `textoize: alt` is used then prefixes
   in the branchname are also translated form "v1.2-333.branchname" is
   applied.
