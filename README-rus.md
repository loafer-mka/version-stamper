...
- [Улучшения version-stamper](#C5)
  ...
  - [Тестирование](#C5.2)
- [Приложения](#C6)
  - [Немного о перехватчиках git (git hooks)](#C6.1)
  - [Обзор структуры каталогов репозитория git](#C6.2)
...
- Makefile скрипт (определяется набор символов)

- Matlab скрипт (определяется класс с константными полями)

- Bash или Sh скрипт (определяется набор символов)

- Windows cmd скрипт (.bat, определяется набор символов)
...
штамплв, эти идентфиикаторы можно модифицировать, дописав к ним специфичные
...
путём редактирования конфигурациолнного файла .version-stamp в корне
рабочего дерева проекта.

# Установка и использование <a name="C4"/>

Для использования version-stamper'а достаочно обеспечить возможность
выполнения основного скрипта version-stamper из каталогов вашего пректа.
...
- только приводящие к оображению сведений (например, `--help`)

- создающие файлы со штампами версий или конфигурации

Первая группа параметов не имеет побочных эффектов, а вторая - приводит
...
При удалении version-stamp надо убедиться, что удалён его конфигурационный
...
Все перехватчикм, принадлежащие version-stamper, содержат строку
...
- `-v` или `--verbose`<br/>
  Выводить отлдочную информацию в процессе выполнения version-stamper'а.
  Информация выводится на `stderr`.

- `-q` или `--quiet`<br/>
  Отключить "говорливый" режим, включенный парраметром `--verbose`.
...
```
  ...
  версий, 333 - номер сбборки (расстояние от текущего коммита до тэга),
  эти сведения возвращает `git describe`. Код версии `VERSION_ID`, равный
  0102014D, это 32-х битовое число, содержащее в старшем байте старший
  номер версии, в следующем байте - младший номер, а в младшем 16-ти
  битовом слове - номер сборки. Имя текущей ветви `branchname` - либо имя
  присоединённой ветви, либо вычисляемое, если HEAD отсоединён (что часто
  бывает для подмодулей). Вычисления не могут гарантировать правильность
  вычисления или выбора, если данному коммиту может соответствовать
  несколько подходящих ветвей.
...
- `-c` или `--config`<br/>
  ...
  изменить то последующая команда генерации штампов обновит перехватчики в
  соответствии с новой конфигурацией. Перехватчики также будут обновляться 
  при обновлении утилиты version-stamper.

- `-s` или `--setup`<br/>
  Установить перехватчики, указанные в конфигурационном файле. Выполнение
  команд --generate или --config подразумевает также установку перехватчиков.
  Данная команда используется для принудительного обновления перехватчиков.

- `-g` или `--generate`<br/>
  Выполнить все плагины, сконфигурированные в .version-stamper.
...
- `--git-hook GITHOOK`<br/>
  ...
  использовать с помощью соответствующих перехватчиков git.

## Команды version-stamper <a name="C4.2"/>

Наряду с перечисленными выше параметрами команджная строка может содержать
...
файле, то включение этого штампа в `.gitignore` или в `.gitattributes`
определяется конфигурацией, а не командной строкой.
...
вы их используете, то предцусмотрите выполнени необходимых команд в своих
...
1. `verbose` - режим говорливости. Значение true соответствует параметру
   `--verbose` в командной строке, а false - `--quiet`. Параметры
   `--verbose` и `--quiet` (и их короткие формы) в командной строке имеют
   приоритет над параметром конфигурации `verbose`.
...
7. Список генерируемых по умолчанию штампов. Отличается от команд командной
  ...
  мал, а в конфгурации параметры не имеют чётко выделяющего их признака).
  <br/>
  <br/>
...
8. `hooks` перечисляет активируемые перехватчики git (git hooks). Возможно
  ...
  изменениея в репозиторий и обновляюься текущие сведения - хэши коммитов,
...
- `pre-commit`
  ...
  неудачно: SHA содержит сведения о комите-предке, а не о текущем коммите.
...
- `post-commit`
  ...
  отслеживаютмя репозиторием, до рабочее дерево становится изменённым (что
...
- `post-checkout`
  ...
  неудачно: переключенние ветки или переход в "отсоединённый" режим
  (detached head) изменяет текущую ветвь и требует обязатальной перегенерации
...
- `post-rewrite`
  ...
  неудачно: штампы версий обновляются после создания коммита, и если они
  отслеживаютмя репозиторием, то рабочее дерево становится изменённым (что
  выглядит странным, когда после коммита всё равно есть изменения и
...
имя плагина и разестить этот файл рядом с `version-stamper`. Плагин
...
`__PLUGIN_XXX_NOTICE__` в случе полагина для языка скриптов `sh` должна
...
суффикс имён переменых окружения: если возвращённая строка не совпадает с
...
Текст сохраняемого файла подаётся на stdin, а аргумерт функции содержит имя
...
с произвольными префиксом и суффиксом, состящими из букв, цифр или символа
...
окажет вляния). Эта строка разбивается на несколько фрагментов: 1) то,
что находится перед нужным идентфикатором, 2) сам идентификатор, 3) оператор
...
## Немного о перехватчиках git (git hooks) <a name="C6.1"/>
...
последовательного применеия изменений. Термин `distance` следует понимать
...
## Обзор структуры каталогов репозитория git <a name="C6.2"/>

В модельном случае предполагаем, что у нас есть репозиторий git в каталоге
`./MAIN`, в этом репозитории имется подмодуль `SUBMODULE` (путь к каталогу
...
она или нет подмодуль привдится в состояние *detached head*, а переключить
его в присоединённое состояние можено только вручную.
...
Назначенное имя рабочего дерева используется для его идентфиикации в
...
папка   .   .   .   ./SUBMODULE         << "идентификатор" подмодуля - такой же относителный путь.
...
папка   .   .   .   ./00        << каталог для объектов, хеш которых начинается с 00.
папки   .   .   .   .   ...
папка   .   .   .   ./FF        << каталог для объектов, хеш которых начинается с FF.
папка   .   .   .   ./info
