...
имеющих кофигурационный файл `.version-stamper` в корне рабочего дерева.
...
- `--trailer TEXT` или `--trailer? TEXT`<br/>
  ...
  на воспросительный знак, задают соответствующие параметры только если
...
- `--declaration TEXT` или `--declaration? TEXT`<br>
  ...
  которых заканчиваются на воспросительный знак, задают соответствующие
...
вы их используете, то предцусмотрите выполнение необходимых команд в своих
...
7. `textoize` определяет вариант формирования текстового представления
   версии. Если значение не присвоено (закомментировано), то используется
   обычное представление в виде "v1.2.333-branchname", если указано
   значение `default`, то формируемая строка выглядит также, но имя ветки
   `branchname` в некоторых случаях сокращается - вместо некоторых возможных
   префиксов `feature-`, `hotfix-`, `fix-`, `issue-`, `release-` используются
   укороченные варианты `f:`, `x:`, `x:`, `#`, и `R:`. Вариант `textoize: alt`
   также осуществляется трансляцию префиксов в имени ветки, но сверх того
   записывает версию в форме "v1.2-333.branchname".
...
`git describe` предсталвет версию в виде "v1.2-333-gHASHCODE", где не
...
`xxx` — название варивнта). Каждый скрипт прост и может быть записан в
одну строку:
